...
def run(stats_engine, ddf: dask_cudf.DataFrame, target: str, candidates: List[str]) -> dask_cudf.DataFrame:
    # Sample tail, fallback to head - Configurable sample size from dcor_max_samples
    sample_n = getattr(stats_engine, 'dcor_max_samples', 200000)  # Use configurable size, default 200k
